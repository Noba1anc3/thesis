...
from layoutlm.data.funsd_knn import FunsdDataset
from layoutlm.modeling.layoutlm_nnc import LayoutlmConfig, LayoutlmForTokenClassification
...
def evaluate(args, model, tokenizer, labels, pad_token_label_id, mode, prefix=""):
    ...
    for batch in tqdm(eval_dataloader, desc="Evaluating"):
        with torch.no_grad():
            ...
            if args.model_type in ["layoutlm"]:
                inputs["bbox"] = batch[4].to(args.device)
                inputs["knns"] = batch[5].to(args.device)
            inputs["token_type_ids"] = (
